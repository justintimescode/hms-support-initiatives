// Bake tests: the sentiment_* columns must be IDENTICAL across the two
...
const ROWS = [
  ...
  { number: "F-1", account: "Epsilon", work_notes: null, additional_comments: POS_J }, // exercises the additional_comments path
];
...
test("schema: version bumped to '12' and all 20 sentiment columns present, unique", () => {
  assert.equal(SCHEMA_VERSION, "12");
  assert.equal(SENTIMENT_KEYS.length, 20);
...
test("parity: types are identical (no BigInt/Number drift) on the numeric columns", () => {
  for (const r of ROWS) {
    ...
    for (const k of ["sentiment_valence", "sentiment_start", "sentiment_end", "sentiment_risk", "sentiment_chases", "sentiment_unanswered", "sentiment_wait_days"]) {
...
/* ==================== baked == direct gradeFromRow ==================== */

test("baked sentiment fields match a direct gradeFromRow call with the same anchors", () => {
  for (const r of ROWS.filter((r) => r.first_response_time == null)) {
    const baked = pickSentiment(enrichForSql(r, SNAP));
    // Same inputs the pipeline passes: frtMs=null, snapshot=SNAP, no resolution marker.
    assert.deepStrictEqual(baked, gradeFromRow(r, null, SNAP, null), `gradeFromRow mismatch on ${r.number}`);
  }
});
...
test("silent row → scoreable:false, null scalars, false hygiene, no undefined columns", () => {
  ...
  for (const k of ["sentiment_valence", "sentiment_label", "sentiment_start", "sentiment_end", "sentiment_arc", "sentiment_signals", "sentiment_quote", "sentiment_esc_reason", "sentiment_confirm", "sentiment_last_quote", "sentiment_wait_days"]) {
  ...
  assert.equal(out.sentiment_escalated, false);
  assert.equal(out.sentiment_risk, null); // closed → no forward-looking risk
  assert.equal(out.sentiment_chases, 0);
  assert.equal(out.sentiment_unanswered, 0);
...
/* ================== lifecycle-specific column behavior ================= */

test("open case bakes a risk; Resolved bakes a reopen-confirm state; closed bakes neither risk nor factors", () => {
  const open = enrichForSql(ROWS[2], SNAP); // N-1, Open, negative
  assert.ok(typeof open.sentiment_risk === "number" && open.sentiment_risk > 0);

  const sp = enrichForSql(ROWS[1], SNAP); // R-1, Resolved
  assert.ok(["confirmed", "conditional", "pushback", "silent"].includes(sp.sentiment_confirm));
  assert.ok(typeof sp.sentiment_risk === "number");

  const closed = enrichForSql(ROWS[0], SNAP); // P-1, Closed, confirmed
  assert.equal(closed.sentiment_risk, null);
  assert.equal(closed.sentiment_confirm, "confirmed");
});
...
test("Arrow: mixed scored/silent rows build cleanly and round-trip (nullable BIGINT/VARCHAR/BOOLEAN)", () => {
  const scored = enrichForSql(ROWS[2], SNAP); // N-1, open + risk
  ...
  const risk = table.getChild("sentiment_risk");
  assert.equal(Number(risk.get(0)), scored.sentiment_risk);
  assert.equal(risk.get(1), null);
