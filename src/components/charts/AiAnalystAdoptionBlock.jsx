...
export function AiAnalystAdoptionBlock({ rows, minCases = MIN_CASES }) {
  ...
  return (
    <Card>
      ...
      <StackKey data={data} />
      <div style={{ height: Math.max(240, data.length * ROW_H + 40), marginTop: 12 }}>
        <ResponsiveContainer>
          <BarChart data={data} layout="vertical" margin={{ top: 10, right: 56, left: 8, bottom: 0 }}>
            ...
            <YAxis
              ...
              tickLine={{ stroke: T.border }}
            />
            <Tooltip content={<AdoptionTip />} cursor={{ fill: T.surfaceAlt }} />
            {STACK.map((s, i) => (
              <Bar
                key={s.key}
                stackId="ai"
    ...
    <div style={{ display: "flex", gap: 14, marginTop: 8, fontSize: 11, color: T.sub, flexWrap: "wrap" }}>
      {present.map((s) => (
        <span key={s.key} style={{ display: "flex", alignItems: "center", gap: 6 }}>
          <span style={{ width: 12, height: 10, background: s.color, border: `1px solid ${alpha(T.ink, 0.12)}`, borderRadius: 2 }} />
          {s.label}
        </span>
      ))}
    </div>
