  ...
  return (
    <Card>
      <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", flexWrap: "wrap", gap: 8 }}>
        <div>
          <div className="eyebrow">SLA breach forecast · next {HORIZON_DAYS} days</div>
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 12 }}>
        ...
        <HorizonChip label="4–7 days" value={summary.later} tone={T.ok} />
                  ...
                  return (
                    <tr key={r.number} style={{ borderBottom: `1px solid ${T.borderSoft}` }}>
                      ...
                      <td className="mono" style={{ ...TD, color: soon ? T.danger : T.warn, fontWeight: 600 }}>
                        <Clock size={14} strokeWidth={2.25} style={{ verticalAlign: "middle", marginRight: 4 }} />{fmtDuration(timeToBreach)}
                      </td>
...
/* Tag ground: the approved Tint 01 that matches the tone, so the chip is a real
 * token instead of a composited alpha. Text on a red tint is Red Shade 02,
 * which is exactly what T.danger already resolves to. */
const TAG_TINT = { [T.danger]: T.dangerSoft, [T.warn]: T.warnSoft, [T.ok]: T.okSoft };

function HorizonChip({ label, value, tone, icon: Icon }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 12px", borderRadius: T.radiusSm, background: T.surfaceAlt, border: `1px solid ${T.borderSoft}` }}>
      {Icon && <Icon size={14} strokeWidth={2.25} style={{ color: tone }} />}
...
function RiskTag({ label, tone }) {
  return (
    <span className="eyebrow" style={{ marginLeft: 6, padding: "1px 6px", borderRadius: T.radiusSm, background: TAG_TINT[tone] || T.surfaceAlt, color: tone }}>
      {label}
    </span>
  );
}
